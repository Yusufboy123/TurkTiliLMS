...
import { b2CourseDefinition, b2LessonDefinitions, b2SectionDefinition } from './b2-content.js';

const prisma = new PrismaClient();

async function findTeacher() {
  const email = process.env.B2_CONTENT_TEACHER_EMAIL?.trim();
  ...
  if (!teacher) throw new Error(email ? `B2 content teacher not found: ${email}` : 'No active Teacher is available for B2 content.');
  return teacher;
}

const sourceKey = (metadata: Prisma.JsonValue | null): string | null => {
...
async function seedLesson(tx: Prisma.TransactionClient, courseId: string, sectionId: string, teacherId: string, definition: (typeof b2LessonDefinitions)[number]) {
  ...
  const activeBlocks = await tx.lessonContentBlock.findMany({ where: { lessonId: lesson.id, deletedAt: null }, select: { id: true, position: true, title: true, metadata: true } });
  const matchedBlocks = new Set<string>();
  for (const content of definition.contentBlocks) {
    const candidate = activeBlocks.filter((block) => !matchedBlocks.has(block.id)).find((block) => sourceKey(block.metadata) === content.key)
      ?? activeBlocks.filter((block) => !matchedBlocks.has(block.id)).find((block) => normalize(block.title ?? '') === normalize(content.title));
    const data = { blockType: content.blockType as LessonContentBlockType, title: content.title, position: content.position, isRequired: true, isVisible: true, textContent: content.textContent ?? null, metadata: { sourceKey: content.key, interactivePractice: content.practiceItems } as unknown as Prisma.InputJsonObject, deletedAt: null };
    const block = candidate ? await tx.lessonContentBlock.update({ where: { id: candidate.id }, data, select: { id: true } }) : await tx.lessonContentBlock.create({ data: { ...data, lessonId: lesson.id, createdById: teacherId }, select: { id: true } });
...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    const course = await tx.course.upsert({ where: { slug: b2CourseDefinition.slug }, create: { ...b2CourseDefinition, level: CourseLevel.B2, status: CourseStatus.PUBLISHED, createdByUserId: teacher.id, teacherId: teacher.id, publishedAt: new Date() }, update: { ...b2CourseDefinition, level: CourseLevel.B2, status: CourseStatus.PUBLISHED, teacherId: teacher.id, publishedAt: new Date(), deletedAt: null }, select: { id: true } });
    const existing = await tx.courseSection.findFirst({ where: { courseId: course.id, position: b2SectionDefinition.position, deletedAt: null }, select: { id: true } });
    const section = existing ? await tx.courseSection.update({ where: { id: existing.id }, data: { title: b2SectionDefinition.title, description: b2SectionDefinition.description, isPublished: true, deletedAt: null }, select: { id: true } }) : await tx.courseSection.create({ data: { courseId: course.id, ...b2SectionDefinition, isPublished: true, createdById: teacher.id }, select: { id: true } });
    for (const definition of b2LessonDefinitions) await seedLesson(tx, course.id, section.id, teacher.id, definition);
  });
  console.info(`Seeded B2 lessons ${b2LessonDefinitions.length}; practice ${b2LessonDefinitions.reduce((sum, lesson) => sum + lesson.contentBlocks.reduce((n, block) => n + block.practiceItems.length, 0), 0)}; tests ${b2LessonDefinitions.reduce((sum, lesson) => sum + lesson.questions.length, 0)}.`);
}

main().catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'B2 content seed failed.'); process.exitCode = 1; }).finally(async () => prisma.$disconnect());
