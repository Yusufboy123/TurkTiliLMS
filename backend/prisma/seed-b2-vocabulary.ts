...
const sourcePath = process.env.B2_VOCABULARY_SOURCE ?? 'D:\\Turk tili\\Darsliklar\\B2\\04_LUGATLAR\\B2_Lugatlar.html';

async function main() {
  const html = await fs.readFile(path.resolve(sourcePath), 'utf8');
  const match = html.match(/const words=(\[[\s\S]*?\])\s*,S=/u);
  if (!match?.[1]) throw new Error('B2 vocabulary source does not contain words.');
  const words = JSON.parse(match[1]) as SourceWord[];
  if (words.length !== 640 || new Set(words.map((word) => word.id)).size !== words.length || words.filter((word) => word.status === 'NEW').length !== 512 || words.filter((word) => word.status === 'REVIEW').length !== 128) throw new Error('B2 vocabulary source coverage is invalid.');
  const course = await prisma.course.findFirst({ where: { slug: 'turk-tili-b2', status: CourseStatus.PUBLISHED, deletedAt: null }, select: { id: true, lessons: { where: { deletedAt: null }, orderBy: { position: 'asc' }, select: { id: true, position: true } } } });
  if (!course || course.lessons.length !== 16) throw new Error('B2 course lessons are not ready.');
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const word of words) {
      ...
      if (!lessonId) throw new Error(`B2 vocabulary lesson ${word.lesson} has no lesson.`);
  ...
  console.info(`Seeded B2 vocabulary: ${words.length} (${words.filter((word) => word.status === 'NEW').length} NEW, ${words.filter((word) => word.status === 'REVIEW').length} REVIEW).`);
}

main().catch((error: unknown) => { console.error(error instanceof Error ? error.message : 'B2 vocabulary seed failed.'); process.exitCode = 1; }).finally(async () => prisma.$disconnect());
