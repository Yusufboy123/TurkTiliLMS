...
Module 8.6B adds nullable `last_authenticated_at` without backfilling legacy
sessions. It advances only after successful password login or approved step-up
verification; refresh rotation and ordinary activity do not make
authentication recent. The `(id, user_id)` unique key exists specifically so
step-up rows can prove that their session belongs to the same user.
...
Module 8.6B implements both tables with UUIDv7 identities, lowercase 64-character
SHA-256 checks, composite challenge/proof binding FKs, action/target/continuation
checks, bounded attempt/time checks, and `RESTRICT` user/session relationships.
A challenge lifecycle trigger freezes its security bindings, prevents attempt
count rollback or jumps, and prevents verified or locked rows from reopening. A
proof-insert trigger locks and requires a verified, unlocked challenge that
remains unexpired at actual database execution time. An update trigger permits
only one unexpired `consumed_at` transition. No password, raw nonce, or raw
proof column exists.
...
- **Important fields:** `actor_user_id`, `enrollment_id`, `key`, operation,
  ...
  progress versions or nullable `resulting_certificate_id` and
  `resulting_certificate_version` as appropriate, `created_at`, and
  `expires_at`.
...
Module 8.6B preserves the physical table and every Progress row, atomically
extends the enum, replaces only the enrollment FK target with
`course_enrollments.id`, adds the nullable certificate-result columns/FK, and
installs an update guard for insert-once successful receipts. A composite FK
requires a certificate result to belong to the receipt enrollment, and the
result-shape check fixes issuance receipts at version 1 and revocation receipts
at version 2. Expired receipt deletion remains permitted. The migration wraps
the enum/FK transition and all additive objects in one PostgreSQL transaction.
...
Module 8.6B seeds only the repeat-safe `STANDARD_COURSE_COMPLETION` stable
identity. A version is deliberately not seeded until brand assets, bundled font
provenance, and renderer contract inputs are approved.
...
`STANDARD_COURSE_COMPLETION`; Module 8.6B creates its stable identity only.
Version 1 and locale `uz-Latn` remain the required first renderable version, but
are deferred until the renderer/asset approval gate. The implemented database
check allows incomplete DRAFT rows, requires complete deterministic
renderer/font provenance for ACTIVE or RETIRED rows, and a trigger freezes
content after activation while allowing only ACTIVE to RETIRED.
...
Module 8.6B implements the number with the database-owned
`certificate_number_global_seq` and `generate_certificate_number()` default.
The exact evaluation/enrollment/course match is enforced through a composite
FK. V1 stores version 1 for ISSUED and version 2 for the single terminal
REVOKED transition; the lifecycle trigger permits only that transition and
rejects deletion or snapshot mutation.
...
backslash, and parent-traversal forms. Metadata is immutable, and size is
restricted to 1–10,485,760 bytes.
...
Module 8.6B stores `reason_code` as a bounded uppercase machine-code token
rather than inventing an unapproved product vocabulary. The database requires
timestamp, actor, and reason to be either all absent or all present. A later
privacy-admin runtime contract must define the allowed reason catalog before
accepting mutations.
